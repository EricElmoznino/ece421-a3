...
class Part2(unittest.TestCase):
    ...
    def test_1(self):   # part 2.2 q1
        ...
        for title in metrics[0]:
            line_plot(list(range(0, epochs + 1)), [m[title] for m in metrics],
                      ['k = %d' % k for k in ks], 'epochs', title.split(' ')[1],
                      os.path.join('results', '2_1', title + '.png'))
        for i, k in enumerate(ks):
            k_means_plot_2d(self.train, cluster_assignments[i], k,
                            os.path.join('results', '2_1', 'assignments_k=%d.png' % k))
        for k, assignments, m in zip(ks, cluster_assignments, metrics):
            with open(os.path.join('results', '2_1', 'final_metrics_k=%d.txt' % k), 'w') as f:
                for title in m:
                    f.write('%s: %g\n' % (title, m[title][-1]))
                x_assigned = [self.train[assignments == i] for i in range(k)]
                assigned_percentages = [len(a) / len(self.train) for a in x_assigned]
    ...
    def test_2(self):
        ...
        for i, k in enumerate(ks):
            k_means_plot_2d(self.val, cluster_assignments_val[i], k,
                            os.path.join('results', '2_2', 'validation_assignments_k=%d.png' % k))
        for k, assignments, m in zip(ks, cluster_assignments_val, metrics):
            with open(os.path.join('results', '2_2', 'final_metrics_k=%d.txt' % k), 'w') as f:
                for title in m:
                    f.write('%s: %g\n' % (title, m[title][-1]))
                x_assigned = [self.val[assignments == i] for i in range(k)]
                assigned_percentages = [len(a) / len(self.val) for a in x_assigned]
    ...
    def test_3(self): #part2.2 q3 km
        ...
        for title in metrics[0]:
            line_plot(list(range(0, epochs + 1)), [m[title] for m in metrics],
                      ['k = %d' % k for k in ks], 'epochs', title.split(' ')[1],
                      os.path.join('results', '2_3', '2_3_1', title + '.png'))
        for i, k in enumerate(ks):
            k_means_plot_2d(self.train, cluster_assignments[i], k,
                            os.path.join('results', '2_3', '2_3_1', 'assignments_k=%d.png' % k))
        for k, assignments, m in zip(ks, cluster_assignments, metrics):
            with open(os.path.join('results', '2_3', '2_3_1', 'final_metrics_k=%d.txt' % k), 'w') as f:
                for title in m:
                    f.write('%s: %g\n' % (title, m[title][-1]))
                x_assigned = [self.train[assignments == i] for i in range(k)]
                assigned_percentages = [len(a) / len(self.train) for a in x_assigned]
    ...
    def test_4(self): #part 2.2 q3 MOG
        ...
        for title in metrics[0]:
            line_plot(list(range(0, epochs + 1)), [m[title] for m in metrics],
                      ['k = %d' % k for k in ks], 'epochs', title.split(' ')[1],
                      os.path.join('results', '2_3',  '2_3_2', title + '.png'))
        for i, k in enumerate(ks):
            k_means_plot_2d(self.train, cluster_assignments[i], k,
                            os.path.join('results','2_3',  '2_3_2', 'assignments_k=%d.png' % k))
        for k, assignments, m in zip(ks, cluster_assignments, metrics):
            with open(os.path.join('results', '2_3', '2_3_2', 'final_metrics_k=%d.txt' % k), 'w') as f:
                for title in m:
                    f.write('%s: %g\n' % (title, m[title][-1]))
                x_assigned = [self.train[assignments == i] for i in range(k)]
                assigned_percentages = [len(a) / len(self.train) for a in x_assigned]
